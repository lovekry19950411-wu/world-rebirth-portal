...
export default function Home() {
  ...
  return (
    <div style={{ backgroundColor: '#05070a', color: '#f8fafc', minHeight: '100vh', fontFamily: 'monospace' }}>
      <Head>
        <title>Sapphire Airdrop | 藍寶石空投轉盤</title>
      </Head>
      ...
      <main style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '20px' }}>
        <div style={{ width: '100%', maxWidth: '450px', background: 'rgba(15, 17, 26, 0.95)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '40px', padding: '40px', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '30px', alignItems: 'flex-start' }}>
            <div>
              <h1 style={{ fontSize: '24px', fontWeight: 900, margin: 0, letterSpacing: '-1px' }}>
                藍寶石<span style={{ color: '#3b82f6' }}>空投轉盤</span> 💎
              </h1>
            ...
            >
              {({ open }: { open: () => void }) => (
                <button 
                  onClick={open} 
                  disabled={isVerified} 
                  style={{ width: '100%', padding: '20px', backgroundColor: isVerified ? '#1e293b' : '#3b82f6', borderRadius: '18px', color: 'white', fontWeight: 900, cursor: isVerified ? 'default' : 'pointer', border: 'none', fontSize: '16px', transition: 'all 0.3s' }}
                >
          ...
          <section style={{ marginBottom: '30px', opacity: isVerified ? 1 : 0.3 }}>
            <form onSubmit={handleInfoSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <input 
                ...
                style={{ width: '100%', padding: '16px', borderRadius: '14px', border: '1px solid #1e293b', background: '#0f172a', color: 'white', fontSize: '14px' }} 
              />
              <input 
                type="text" 
                placeholder="X (Twitter) 帳號 @..." 
                value={twitter} 
              ...
              <button 
                type="submit" 
                disabled={!isVerified || isSubmitting} 
                style={{ width: '100%', padding: '14px', backgroundColor: 'transparent', border: '1px solid #3b82f6', borderRadius: '14px', color: '#3b82f6', fontWeight: 'bold', cursor: 'pointer' }}
              >
          ...
          <section style={{ textAlign: 'center', background: 'radial-gradient(circle, rgba(59,130,246,0.1) 0%, rgba(5,7,10,0) 70%)', padding: '40px 20px', borderRadius: '35px', border: '1px solid rgba(59,130,246,0.1)', opacity: isVerified ? 1 : 0.3 }}>
            <div style={{ fontSize: '70px', marginBottom: '25px', animation: isSpinning ? 'spin 1s infinite linear' : 'none', display: 'inline-block', filter: 'drop-shadow(0 0 15px #3b82f6)' }}>
              💎
            </div>
            <button 
              onClick={startSpin} 
              disabled={!isVerified || isSpinning} 
              style={{ width: '100%', padding: '20px', background: isVerified ? 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)' : '#1e293b', border: 'none', borderRadius: '18px', color: 'white', fontWeight: 900, fontSize: '18px', boxShadow: isVerified ? '0 10px 30px rgba(37,99,235,0.4)' : 'none', cursor: 'pointer' }}
            >
          ...
          <div style={{ minHeight: '60px', marginTop: '25px' }}>
            {message && (
              <div style={{ padding: '15px', backgroundColor: 'rgba(59,130,246,0.1)', borderRadius: '15px', fontSize: '12px', textAlign: 'center', color: '#93c5fd', border: '1px solid rgba(59,130,246,0.2)' }}>
                {message}
              </div>
            )}
          </div>
